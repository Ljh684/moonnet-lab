...
- 场景文件：拓扑、流量、参数、种子写成可读配置，一次实验可以原样重放
- `compare`：多算法正面对比；`sweep`：参数扫描
- 指标导出 JSON（吞吐、RTT、窗口轨迹、队列占用、重传率、Jain 公平性指数）
- 自渲染 SVG 图表

验收方式：同一场景重复运行导出的 JSON 逐字节一致；多流公平性实验能复现出"无拥塞控制时总吞吐随流数下降"的结论。
